...
export function createApiRoutes() {
  ...
  app.get("/api/strategy", async (c) => {
    try {
      ...
      const strategyNames: Record<string, string> = {
        ...
        "aggressive-team": "Aggressive Team",
        "rebate-farming": "Rebate Farming",
  ...
  app.post("/api/close-position", async (c) => {
    try {
      ...
      // 验证必填参数
      if (!symbol) {
        return c.json({ success: false, message: "Missing required parameter: symbol" }, 400);
      }
      ...
      // 如果未配置密码，拒绝平仓
      if (!correctPassword) {
        logger.error('Close position password not configured - set CLOSE_POSITION_PASSWORD');
        return c.json({ 
          success: false, 
          message: "Close position is disabled" 
        }, 403);
      }
      
      if (!password || password !== correctPassword) {
        logger.warn(`Close position password validation failed - symbol: ${symbol}`);
      ...
      logger.info(`Manual close ${symbol} ${side === "long" ? "Long" : "Short"} ${quantity} contracts (entry: ${entryPrice.toFixed(2)}, current: ${currentPrice.toFixed(2)}, P&L: ${pnl.toFixed(2)})`);
      ...
      logger.info(`Manual close order placed for ${symbol}, order ID: ${order.id}`);
      ...
      if (order.id) {
        try {
          ...
          logger.warn(`Failed to fetch order info: ${error.message}`);
        }
      }
      ...
      // 记录到交易历史
      try {
        logger.info(`Preparing to record close trade: ${symbol}, order ID: ${order.id || `manual_${Date.now()}`}`);
        ...
        logger.info(`✓ Trade history saved, record ID: ${insertResult.lastInsertRowid}`);
      } catch (dbError: any) {
        logger.error(`✗ Failed to record trade history: ${dbError.message}`, dbError);
      ...
      try {
        ...
        logger.info(`✓ Position record deleted: ${symbol}, rows affected: ${deleteResult.rowsAffected}`);
      } catch (dbError: any) {
        logger.error(`✗ Failed to delete position record: ${dbError.message}`, dbError);
        // 这里不抛出错误，因为交易已经完成
      }
      
      logger.info(`Manual close completed ${symbol}, P&L: ${actualPnl.toFixed(2)} USDT`);
      ...
      logger.error("Manual close failed:", error);
      return c.json({ 
        success: false, 
